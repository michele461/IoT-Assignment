...
float zHistory[MAX_WINDOW];
int zIdx = 0;

void applyZScore(SensorData &d) {
    float sum = 0, mean = 0, variance = 0;
    for(int i=0; i<DYNAMIC_WINDOW; i++) sum += zHistory[i];
    mean = sum / DYNAMIC_WINDOW;
    
    for(int i=0; i<DYNAMIC_WINDOW; i++) {
        float diff = zHistory[i] - mean;
        variance += diff * diff;
    }
    variance /= DYNAMIC_WINDOW;

    if (variance > 0.01) {
        float diff = abs(d.rawValue - mean);
        if ((diff * diff) > (9.0 * variance)) {
    ...
    zIdx = (zIdx + 1) % DYNAMIC_WINDOW;
}

float hBuffer[MAX_WINDOW];
int hIdx = 0;

void applyHampel(SensorData &d) {
    hBuffer[hIdx] = d.rawValue;
    hIdx = (hIdx + 1) % DYNAMIC_WINDOW;

    float temp[MAX_WINDOW];
    memcpy(temp, hBuffer, sizeof(float) * DYNAMIC_WINDOW);
    
    for(int i=0; i<DYNAMIC_WINDOW-1; i++) {
        for(int j=i+1; j<DYNAMIC_WINDOW; j++) {
            if(temp[i] > temp[j]) { float t = temp[i]; temp[i] = temp[j]; temp[j] = t; }
        }
    }
    float median = temp[DYNAMIC_WINDOW / 2];

    float deviations[MAX_WINDOW];
    for(int i=0; i<DYNAMIC_WINDOW; i++) deviations[i] = abs(hBuffer[i] - median);
    ...
    float mad = deviations[DYNAMIC_WINDOW / 2];
...
void setupInitialState() {
    ...
    else if (RUN_MODE == 4) { ADAPTIVE_SAMPLING = true; SIGNAL_TYPE = 1; ENABLE_NOISE = true; FILTER_TYPE = 1; ANOMALY_PROB = 0.01; DYNAMIC_WINDOW = 3; }
    else if (RUN_MODE == 5) { ADAPTIVE_SAMPLING = true; SIGNAL_TYPE = 1; ENABLE_NOISE = true; FILTER_TYPE = 2; ANOMALY_PROB = 0.01; DYNAMIC_WINDOW = 3; }
...
void updatePhaseState(int phase) {
    if (RUN_MODE == 1 || RUN_MODE == 2) {
        ...
        if (phase == 1) { ANOMALY_PROB = 0.01; DYNAMIC_WINDOW = 7; }
        else if (phase == 2) { ANOMALY_PROB = 0.01; DYNAMIC_WINDOW = 15; }
        else if (phase == 3) { ANOMALY_PROB = 0.05; DYNAMIC_WINDOW = 3; }
        else if (phase == 4) { ANOMALY_PROB = 0.05; DYNAMIC_WINDOW = 7; }
        else if (phase == 5) { ANOMALY_PROB = 0.05; DYNAMIC_WINDOW = 15; }
        else if (phase == 6) { ANOMALY_PROB = 0.10; DYNAMIC_WINDOW = 3; }
        else if (phase == 7) { ANOMALY_PROB = 0.10; DYNAMIC_WINDOW = 7; }
        else if (phase == 8) { ANOMALY_PROB = 0.10; DYNAMIC_WINDOW = 15; }
    }
}
...
void TaskAnalyze(void *pvParameters) {
    ...
    int maxPhases = 3;
    if (RUN_MODE == 3) maxPhases = 1;
    else if (RUN_MODE >= 4) maxPhases = 9;
    ...
    for (;;) {
        if (millis() - phaseStartTime >= PHASE_DURATION_MS) {
            ...
            zIdx = 0; hIdx = 0;
            for(int i=0; i<MAX_WINDOW; i++) zHistory[i] = 0;
            for(int i=0; i<MAX_WINDOW; i++) hBuffer[i] = 0;
            adaptiveDone = false;
            sampleCount = 0;
            currentDelayMs = 1000 / INITIAL_SAMPLING_HZ;
        ...
        if (xQueueReceive(sampleQueue, &d, portMAX_DELAY) == pdPASS) {
            ...
            if (millis() - windowStartTime >= WINDOW_MS) {
                if (windowCount > 0) {
                    ...
                    Serial0.println("\n=== 5s WINDOW REPORT ===");
                    Serial0.printf("[PARAMS] Sig: %d | Adaptive: %s | Noise: %s | Filter: %d | Prob: %.2f | Win: %d\n", SIGNAL_TYPE, ADAPTIVE_SAMPLING ? "YES" : "NO", ENABLE_NOISE ? "YES" : "NO", FILTER_TYPE, ANOMALY_PROB, DYNAMIC_WINDOW);
